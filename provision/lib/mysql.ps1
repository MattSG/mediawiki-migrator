...
"@
    ...
    if (-not (Test-Path $iniPath)) {
        ...
        $mysqlDirForConfig = $Script:MysqlDir -replace '\\', '/'
        $dataDirForConfig = $dataDir -replace '\\', '/'
        @"
[mysqld]
port=$DbPort
basedir=$mysqlDirForConfig
...
"@ | Set-Content $iniPath
    ...
    $rootAuthArgs = @('--host=127.0.0.1', "--port=$DbPort", '-uroot', '--skip-password')
    if ($DbRootPassword) {
        & $mysqlExe '--host=127.0.0.1' "--port=$DbPort" -uroot "-p$DbRootPassword" -e 'SELECT 1' 2>$null | Out-Null
        if ($LASTEXITCODE -eq 0) { $rootAuthArgs = @('--host=127.0.0.1', "--port=$DbPort", '-uroot', "-p$DbRootPassword") }
    }
