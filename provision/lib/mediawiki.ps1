...
function Get-GitHubZip {
    ...
    # VisualEditor fetches its ve submodule into a second target; keep that archive
    # separate from the extension archive (same repo/branch basename).
    $suffix = if ((Split-Path $TargetDir -Leaf) -eq 've') { '-ve' } else { '' }
    $zip = Join-Path $Script:DownloadDir "$Repo-$Branch$suffix.zip"
...
`$wgDisableTalk      = true;
`$wgHooks['SkinTemplateNavigationUniversal'][] = static function (`$skin, &`$links) { unset(`$links['associated-pages']['talk']); };
`$wgHooks['BeforePageDisplay'][] = static function (`$out) { `$out->addInlineStyle('#ca-talk{display:none!important}'); };
...
`$wgFileExtensions    = array_merge( `$wgFileExtensions, [ 'png', 'jpg', 'jpeg', 'gif', 'svg', 'pdf', 'webp', 'doc', 'docx', 'xls', 'xlsx', 'ppt', 'pptx', 'odt', 'ods', 'odp', 'csv' ] );
...
`$wgVisualEditorEnableVisualEditor = true;
`$wgVisualEditorEnableWikitext = true;
`$wgVisualEditorAvailableNamespaces = [ 'Main' => true, 'User' => true, 'File' => true, 'MediaWiki' => true, 'Template' => true, 'Help' => true, 'Category' => true, 'Wholesale_Support' => true ];
"@
    }
    ...
    foreach ($ext in @('SemanticResultFormats', 'Mermaid')) {
        if ((Test-Path (Join-Path $Script:WwwDir "extensions\$ext")) -and ($block -notmatch "wfLoadExtension\( '$ext' \)")) {
            $block += "`nwfLoadExtension( '$ext' );"
        }
    }
    if ($InstallPdfTools -and (Test-Path (Join-Path $Script:WwwDir 'extensions\PdfHandler'))) {
